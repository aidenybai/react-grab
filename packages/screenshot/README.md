...
Every change runs against a Playwright harness ([`e2e/fidelity.spec.ts`](./e2e/fidelity.spec.ts)) that captures each fixture with `captureNode` **and** with the browser's native `locator.screenshot()`, then compares the two with `pixelmatch`. The score is `diffPixels / (width × height)`; each fixture has a budget in [`e2e/fixture-manifest.ts`](./e2e/fixture-manifest.ts). A second assertion bounds the mean per-channel delta across the whole image, so uniform color drift below pixelmatch's per-pixel threshold still fails.

The suite runs on three engines — Chromium, WebKit, and Firefox — with per-engine budget overrides in the manifest where an engine's own rasterization quirks (glyph antialiasing inside SVG images, native form/media chrome, headless Firefox skipping `backdrop-filter` compositing in the ground-truth screenshot) justify a documented loosening.

The suite holds 406 fixtures: 21 written for this library (gradients, transforms, typography with bundled webfonts, flex/grid, shadows, filters, images, pseudo-elements, form controls, scrolled block/flex/grid containers, shadow DOM, canvas, a kitchen sink, and a ~1460-node stress page), 109 ported from the test suites of snapdom (`snap-`), modern-screenshot (`ms-`), html-to-image (`hti-`), and dom-to-image-more (`dtim-`) — CSS counters, icon sprites, SVG `<use>`/defs, sticky-in-scroll, `::first-letter`, same-origin iframes, srcset, tables, custom elements, and the rest of their visual regression catalogs — plus 12 limitation-workaround fixtures (`lim-`) covering `::marker` styling, indeterminate checkboxes, offscreen lazy images, offscreen `content-visibility: auto`, cross-origin `@font-face` stylesheets, `position: fixed` inside scrolled pages, transformed roots, `bleed` around box-shadows, `backdrop-filter`, and cross-origin iframes via the postMessage bridge, 10 hard edge-case fixtures (`hard-`) — CSS and WAAPI animations paused mid-keyframe, stacked and rotated `backdrop-filter` glass, blend modes with `clip-path` and masks, vertical/RTL writing modes, styled and nested cross-origin iframes, and a sticky+fixed+bleed stress combo at scale×DPR — and 3 large realistic pages (`real-`) modeled on open-source UI patterns: a repo file browser, an analytics dashboard, and a glass-morphism landing page — and 250 deterministic real-website pages (`site-`): 25 archetypes (repo view, social feed, video grid, product page, pricing, wiki article, link aggregator, comment thread, email inbox, team chat, kanban board, music player, streaming rows, search results, blog article, profile page, listing grid, checkout form, analytics dashboard, docs site, calendar, Q&A thread, newspaper front page, settings page, landing hero) each in ten theme palettes (light, dark, sepia, midnight, ocean, forest, rose, violet, amber, slate). On Chromium, 398 fixtures hold the strict 0.005 budget (most score exactly 0 — no pixel differs above pixelmatch's 10% YIQ color threshold); the exceptions are 4 form-control fixtures at 0.05, where native widget chrome legitimately rasterizes differently inside `foreignObject`, the indeterminate-form fixture ratcheted to 0.008, and 3 transformed-root fixtures at 0.011 where diagonal edges are antialiased twice.
...
`npm run bench` ([`bench/benchmark.spec.ts`](./bench/benchmark.spec.ts)) compares `captureNode` against snapdom, modern-screenshot, html-to-image, html2canvas, and dom-to-image-more on ten fixtures (six synthetic pages plus the four heaviest real-website pages). Cold start is reported as the median of 5 captures taken on separate fresh page loads; warm timings are the median and p95 of 8 subsequent captures in the same page. Every capture is also fidelity-scored against Chromium's native screenshot, so read the speed columns together with the fidelity column — a fast capture that renders the wrong pixels is not comparable to an accurate one. Timings still vary run to run; treat small gaps between libraries as noise rather than a ranking. Results are written to `test-results/bench-results.json`.

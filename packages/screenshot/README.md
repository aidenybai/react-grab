...
The suite holds 155 fixtures: 21 written for this library (gradients, transforms, typography with bundled webfonts, flex/grid, shadows, filters, images, pseudo-elements, form controls, scrolled block/flex/grid containers, shadow DOM, canvas, a kitchen sink, and a ~1460-node stress page), 109 ported from the test suites of snapdom (`snap-`), modern-screenshot (`ms-`), html-to-image (`hti-`), and dom-to-image-more (`dtim-`) — CSS counters, icon sprites, SVG `<use>`/defs, sticky-in-scroll, `::first-letter`, same-origin iframes, srcset, tables, custom elements, and the rest of their visual regression catalogs — plus 12 limitation-workaround fixtures (`lim-`) covering `::marker` styling, indeterminate checkboxes, offscreen lazy images, offscreen `content-visibility: auto`, cross-origin `@font-face` stylesheets, `position: fixed` inside scrolled pages, transformed roots, `bleed` around box-shadows, `backdrop-filter`, and cross-origin iframes via the postMessage bridge, 10 hard edge-case fixtures (`hard-`) — CSS and WAAPI animations paused mid-keyframe, stacked and rotated `backdrop-filter` glass, blend modes with `clip-path` and masks, vertical/RTL writing modes, styled and nested cross-origin iframes, and a sticky+fixed+bleed stress combo at scale×DPR — and 3 large realistic pages (`real-`) modeled on open-source UI patterns: a repo file browser, an analytics dashboard, and a glass-morphism landing page. On Chromium, 147 fixtures hold the strict 0.005 budget (most score exactly 0 — no pixel differs above pixelmatch's 10% YIQ color threshold); the exceptions are 4 form-control fixtures at 0.05, where native widget chrome legitimately rasterizes differently inside `foreignObject`, the indeterminate-form fixture ratcheted to 0.008, and 3 transformed-root fixtures at 0.011 where diagonal edges are antialiased twice.

...
const ruleDefinition = {
  ...
  createOnce(context) {
    ...
    return {
      ...
      CallExpression(node) {
        if (node.callee?.type === "Identifier" && tracker.matchImport(["createEffect", "createMemo"], node.callee.name) && node.arguments?.length === 2 && node.arguments.every((arg) => arg.type !== "SpreadElement")) {

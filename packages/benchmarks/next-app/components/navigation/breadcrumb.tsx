      ...
      <ol style={{ display: "flex", alignItems: "center", gap: 4, listStyle: "none", margin: 0, padding: 0 }}>
        {displayItems.map((item, i) => (
          <React.Fragment key={i}>
            {i > 0 && (
              <li style={{ color: "#9CA3AF", display: "flex" }}>{separator || defaultSeparator}</li>
            )}
            <li>
              {item.href && i < displayItems.length - 1 ? (
                ...
                <span style={{ fontSize: 13, color: i === displayItems.length - 1 ? "#111827" : "#6B7280", fontWeight: i === displayItems.length - 1 ? 500 : 400 }}>
                  {item.label}
                </span>
              )}
            </li>

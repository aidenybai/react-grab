...
export function ActionMenu({ items, trigger, align = "right" }: ActionMenuProps) {
  ...
  return (
    <div ref={menuRef} className="relative inline-block">
      <button onClick={() => setOpen(!open)} className="p-1 rounded hover:bg-gray-100 text-gray-500">
        {trigger || (
      ...
      {open && (
        <div className={`absolute z-50 mt-1 w-48 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 py-1 ${align === "right" ? "right-0" : "left-0"}`}>
          {items.map((item, i) => (

...
interface OrgSsoConfigProps {
  ...
  ssoUrl?: string;
  certificate?: string;
  onSave: (config: { provider: string; entityId: string; ssoUrl: string; certificate: string }) => void;
      ...
      {isEnabled && (
        <div className="org-sso-config__form">
          ...
          <div className="org-sso-config__field">
            ...
            <input type="text" value={formEntityId} onChange={(e) => setFormEntityId(e.target.value)} />
          </div>
          <div className="org-sso-config__field">
            <label>SSO URL</label>
            <input type="url" value={formSsoUrl} onChange={(e) => setFormSsoUrl(e.target.value)} />
          </div>
          <div className="org-sso-config__field">
            <label>Certificate</label>
            <textarea value={formCertificate} onChange={(e) => setFormCertificate(e.target.value)} rows={4} />
          </div>
          <button className="btn btn-primary" onClick={() => onSave({ provider: formProvider, entityId: formEntityId, ssoUrl: formSsoUrl, certificate: formCertificate })}>

...
interface AvailabilityOverrideDialogProps {
  isOpen: boolean;
  date?: Date;
  onSave: (override: { date: string; ranges: Array<{ start: string; end: string }> }) => void;
  onClose: () => void;
}
...
export function AvailabilityOverrideDialog({
  ...
  return (
    <div className="availability-override-dialog__overlay" onClick={onClose}>
      <div className="availability-override-dialog" onClick={(e) => e.stopPropagation()}>
        <h2>Date Override</h2>
        ...
        <div className="availability-override-dialog__actions">
          ...
          <button className="btn btn-primary" onClick={() => onSave({ date: dateStr, ranges: isUnavailable ? [] : ranges })}>
            Save Override
          </button>
        </div>
      </div>

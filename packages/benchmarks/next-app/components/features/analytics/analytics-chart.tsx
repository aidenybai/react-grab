          ...
          {data.map((point, index) => (
            <div key={index}>
              <div className="mb-1 flex items-center justify-between text-xs">
                ...
                {showValues && <span className="text-gray-500">{point.value.toLocaleString()}</span>}
              </div>
        ...
        {data.map((point, index) => {
          ...
          return (
            <div key={index} className="flex flex-1 flex-col items-center gap-1">
              ...
              />
              {showLabels && (
                <span className="mt-1 truncate text-[10px] text-gray-400" style={{ maxWidth: maxBarWidth }}>
                  {point.label}
                </span>
              )}
            </div>

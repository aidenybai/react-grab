...
function StatCard({
  ...
  return (
    ...
    >
      ...
      <p className="text-2xl font-bold mt-1 text-[var(--foreground)]">{value}</p>
...
function MiniTable({ "data-testid": testId }: { "data-testid"?: string }) {
  ...
  return (
    <div className="rounded-lg border border-[var(--border)] overflow-hidden" data-testid={testId}>
      <table className="w-full text-sm">
        <thead>
          <tr className="bg-[var(--muted)]">
            <th className="text-left px-4 py-2 font-medium text-[var(--muted-foreground)]">Product</th>
            <th className="text-left px-4 py-2 font-medium text-[var(--muted-foreground)]">Sales</th>
        ...
        <tbody>
          {rows.map((row) => (
            <tr key={row.name} className="border-t border-[var(--border)]">
              ...
              <td className="px-4 py-2 text-[var(--foreground)]">{row.sales.toLocaleString()}</td>
...
export function TwDashboard({ "data-testid": testId }: { "data-testid"?: string }) {
  return (
    <div data-testid={testId} className="space-y-4">
      <div className="grid grid-cols-4 gap-4">
        <StatCard label="Revenue" value="$45.2K" change="+12.5%" data-testid="tw-stat-revenue" />

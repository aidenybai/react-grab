...
const MoreOptionsButton: Component<MoreOptionsButtonProps> = (props) => {
  return (
    <button
      ...
      class="flex items-center justify-center size-[18px] rounded-sm cursor-pointer bg-transparent hover:bg-white/10 text-[#A7A7A7] hover:text-white border-none outline-none p-0 shrink-0 press-scale"
    ...
    >
      <Show when={!didCopy() && props.onDismiss}>
        <div class="contain-layout shrink-0 flex items-center justify-between gap-2 pt-1.5 pb-1 px-2 w-full h-fit">
          <span class="text-white text-[13px] leading-4 font-sans font-medium h-fit tabular-nums overflow-hidden text-ellipsis whitespace-nowrap min-w-0">
            {displayStatusText()}
          </span>
          <div class="contain-layout shrink-0 flex items-center gap-2 h-fit">
            ...
            <Show when={props.onDismiss}>
              <button
                ...
                disabled={didCopy()}
              >
                <span class="text-white text-[13px] leading-3.5 font-sans font-medium">Keep</span>
                <Show when={!didCopy()}>
                  <IconReturn size={10} class="text-[#A7A7A7]" />
                </Show>
              </button>
            </Show>
          </div>
      ...
      <Show when={didCopy() || !props.onDismiss}>
        <div class="contain-layout shrink-0 flex items-center gap-0.5 py-1.5 px-2 w-full h-fit">
          <IconCheck size={14} class="text-white/85 shrink-0 animate-success-pop" />
          <span class="text-white text-[13px] leading-4 font-sans font-medium h-fit tabular-nums overflow-hidden text-ellipsis whitespace-nowrap min-w-0">
            {displayStatusText()}
          </span>
          <Show when={props.onShowContextMenu}>

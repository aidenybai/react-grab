...
interface GrabDemoController {
  ...
  /** The React Grab shadow root, for querying overlay internals. */
...
export const createGrabDemo = (options: GrabDemoOptions): GrabDemoController => {
  ...
  // Bumped by cancel()/dispose() so the multi-step typeText loop aborts;
  // single-shot waits/animations are torn down via settlePending().
  let cancelToken = 0;
  ...
  const pointerDown = (x: number, y: number): void => dispatchPointer("pointerdown", x, y, 1);
  const pointerUp = (x: number, y: number): void => dispatchPointer("pointerup", x, y, 0);
  ...
  const getShadowRoot = (): ShadowRoot | null =>
    document.querySelector(`[${REACT_GRAB_HOST_ATTRIBUTE}]`)?.shadowRoot ?? null;
...
export type { GrabDemoOptions, GrabDemoController, GrabDemoPoint };

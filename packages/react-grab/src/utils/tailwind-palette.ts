import { hexToHsl } from "./parse-color.js";
...
const hexToRgb = (hex: string): { r: number; g: number; b: number } | null => {
  ...
  const digits = cleaned.length === 8 ? cleaned.slice(0, 6) : cleaned;
...
// Finds the palette entry whose RGB is closest to the input hex
// (squared Euclidean distance). Used as the "anchor" before shift-arrow
// steps through shades — if the user starts on a custom hex the first
// step snaps to the nearest tailwind family then walks from there.
...
export const stepTailwindShade = (hex: string, direction: 1 | -1): string | null => {
  ...
  const nextIndex =
    (currentIndex - direction + TAILWIND_SHADES.length) % TAILWIND_SHADES.length;

  ...
  const createStyleMapProxy = (
    ...
    const proxy = new Proxy(styleMap, {
      get(target, prop) {
        ...
        if (prop === Symbol.iterator) {
          return (
            target as unknown as Iterable<[string, CSSStyleValue]>
          )[Symbol.iterator].bind(target);
        }
        const value = Reflect.get(target, prop) as unknown;
        return typeof value === "function"
          ? (value as (...args: unknown[]) => unknown).bind(target)
  ...
  const createNodeListProxy = (nodeList: NodeList) =>
    new Proxy(nodeList, {
      get(target, property) {
        ...
        const propertyValue = Reflect.get(target, property) as unknown;
        return typeof propertyValue === "function"
          ? (propertyValue as (...args: unknown[]) => unknown).bind(target)
          : propertyValue;
      },
    });

...
The visual highlight overlays are rendered via [components/overlay-canvas.tsx](../src/components/overlay-canvas.tsx) using a single `<canvas>` element. When `transferControlToOffscreen()` is available, the canvas is transferred to a dedicated worker whose compiled source is embedded into the library bundle and started from a blob URL. Browsers without transferable `OffscreenCanvas` support use the same renderer on the main thread, so both paths share their animation and drawing behavior.

The bounds for each visual type are lerped toward their target positions on every animation frame, with different lerp factors for different interaction types. Selection highlights use a slower factor so the overlay doesn't jitter as the user moves between elements, while drag rectangles track the pointer more aggressively. When the animation has converged (the current bounds are within a small threshold of the target bounds and the opacity has settled), the animation loop stops until the next state update triggers it again. Workers without `requestAnimationFrame` use a frame-duration timer fallback.
